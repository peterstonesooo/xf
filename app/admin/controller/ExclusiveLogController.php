...
class ExclusiveLogController extends AuthController
{
    ...
    public function exclusiveLogList()
    {
        ...
        // 每页数量，默认15条
        $limit = isset($req['limit']) && is_numeric($req['limit']) ? intval($req['limit']) : 15;
    ...
    public function auditPass()
    {
        ...
        try {
            ...
            if ($user) {
                // 增加收益金到用户惠民钱包
                User::changeInc($exclusiveLog['user_id'], $req['minsheng_amount'], 'gongfu_wallet', 63, $req['id'], 16, '专属补贴', 0, 1);
            }
    ...
    public function batchAuditPass()
    {
        ...
        $exclusiveLogs = ExclusiveLog::whereIn('id', $req['ids'])
            ->where('status', 0)
            ->select();
        
        if ($exclusiveLogs->isEmpty()) {
            return out(null, 10001, '没有可审核的记录');
        }
