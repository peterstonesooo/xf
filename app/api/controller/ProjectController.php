...
class ProjectController extends AuthController
{
    public function projectList()
    {
        ...
        $data = Project::field('id, name, name_background, intro, cover_img, details_img, 
        single_amount,status, sum_amount, period, support_pay_methods, created_at,
        ...
        remaining_stock,yuding_time,yuding_amount,gongfu_right_now,zhenxing_right_now,minsheng_right_now,
...
//                ->append(['daily_bonus'])
        ...
        foreach($data as $item){
            ...
            if($item['total_stock'] > 0){
                $rate = round(($item['total_stock'] - $item['remaining_stock']) / $item['total_stock'] * 100, 2);
                $item['progress_rate'] = $rate == (int)$rate ? (int)$rate : $rate;
                if($status == 0){
                    $item['progress_rate'] = 0;
                }
            }else{
    ...
    public function projectDetails(){
        ...
        $data = Project::where('id', $req['project_id'])
            ->field('cover_img,name,single_amount,period,huimin_amount,gongfu_amount,minsheng_amount,rebate_rate,total_quota,remaining_quota,daily_bonus_ratio,zhenxing_wallet,puhui,gongfu_right_now,zhenxing_right_now,gold_right_now')->find()->toArray();
