...
class UserController extends AuthController
{
    public function userInfo()
    {
        ...
        $user = User::where('id', $user['id'])
                    ->field('id,phone,realname,pay_password,up_user_id,is_active,invite_code,ic_number,level,balance,topup_balance,team_bonus_balance,appreciating_wallet,butie_lock,created_at,qq,avatar,digit_balance,butie,integral,tiyan_wallet,tiyan_wallet_lock,xingfu_tickets,puhui,zhenxing_wallet,vote_tickets,shouyi_wallet,gongfu_wallet,vip_status,total_pay_amount')
                    ->find()
                    ->toArray();
...
//        }
        ...
        // 从Capital表统计，status IN (1,2,4)表示待审核、审核通过、待打款，排除status=3（审核拒绝）
        // type=2表示提现，amount是负数所以用0减去
        $user['withdrawal_total'] = round(0 - Capital::where('user_id', $user['id'])
            ->whereIn('status', [1, 2, 4]) // 包括待审核(1)、审核通过(2)、待打款(4)，排除审核拒绝(3)
...
//        $data['list'] = [];
    ...
    public function teamRankList()
    {
       $req = $this->validate(request(), [
        ...
        //办理特定项目人数
        // 统计购买项目 191/192/193 的用户数（按用户去重）
        $specific_project_num = Order::whereIn('user_id', $three_realname_ids)
