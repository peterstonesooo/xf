<?php /*a:4:{s:81:"/Applications/XAMPP/xamppfiles/htdocs/xf/app/admin/view/setting/setting_list.html";i:1754294218;s:67:"/Applications/XAMPP/xamppfiles/htdocs/xf/app/admin/view/layout.html";i:1754294218;s:65:"/Applications/XAMPP/xamppfiles/htdocs/xf/app/admin/view/head.html";i:1754294218;s:65:"/Applications/XAMPP/xamppfiles/htdocs/xf/app/admin/view/left.html";i:1754294218;}*/ ?>
<!DOCTYPE html>
<html>
...
<div id="wrapper">
    ...
    <form class="form-inline">
        <input type="text" value="<?php echo isset($req['setting_id']) ? htmlentities($req['setting_id']) : ''; ?>" name="setting_id" placeholder="搜索ID" class="form-control">
        <input type="text" value="<?php echo isset($req['key']) ? htmlentities($req['key']) : ''; ?>" name="key" placeholder="搜索配置key" class="form-control">
                    ...
                    <table class="table table-bordered table-hover table-striped">
                        ...
                        <?php foreach ($data as $k => $v) { ?>
                            <tr>
                                ...
                                <td>
                                    <?php if (in_array($v['key'], ['is_req_encypt', 'auth_switch', 'equity_switch', 'register_give_equity_switch', 'register_give_digital_yuan_switch', 'bank_withdrawal_switch', 'alipay_withdrawal_switch', 'automatic_withdrawal_switch','register_digital_yuan_switch', 'recharge_switch', 'butie_switch', 'red_envelope_switch', 'hongli_switch'])) { ?>
                                        <div class="switch">
                                            <div class="onoffswitch">
                                                <input type="checkbox" <?php echo $v['value'] == 1 ? 'checked' : ''; ?>
                                                       class="onoffswitch-checkbox" id="data<?php echo htmlentities($v['id']); ?>">
                                                <label class="onoffswitch-label" for="data<?php echo htmlentities($v['id']); ?>"
                                                       onclick="changeSetting(<?php echo htmlentities($v['id']); ?>, 'data')">
                                    ...
                                    <?php } else { ?>
                                        <?php echo htmlentities($v['value']); if (in_array($v['key'], ['first_team_reward_ratio', 'second_team_reward_ratio', 'third_team_reward_ratio', 'withdraw_fee_ratio'])) { echo '%'; } } ?>
                                </td>
                                <td>
                                    <?php if (!in_array($v['key'], ['equity_switch', 'register_give_equity_switch', 'register_give_digital_yuan_switch'])) { ?>
                                    <a <?php echo auth_show_judge('Setting/editSetting'); ?> class="btn btn-flat btn-info btn-xs"
                                    href="<?php echo url('admin/Setting/showSetting', ['id' => $v['id']]); ?>"><i
...
<script>
    function changeSetting(id, field)
    {
        ...
        layer.confirm('确定操作吗', {icon: 3, title: '提示'}, function (index) {
            layer.close(index);
            $.post('<?php echo url("admin/Setting/editSetting"); ?>', {"id": id, "value": value}, function (res) {
                if (res.code != 200) {
                    if (value == 1) {
